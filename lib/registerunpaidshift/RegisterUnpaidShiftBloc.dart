...
class RegisterUnpaidShiftBloc extends Bloc<RegisterUnpaidShiftEvent, RegisterUnpaidShiftState> {
  RegisterUnpaidShiftBloc() : super(InitState()) {
    ...
    on<StartDateTimeEvent>((event,emit)=> emit(RegisterUnpaidShiftLoadedState(carerId: (state as RegisterUnpaidShiftLoadedState).carerId, start: event.value.fromColombianToLocalTime(), end: (state as RegisterUnpaidShiftLoadedState).end)));
    on<EndDateTimeEvent>((event,emit)=> emit(RegisterUnpaidShiftLoadedState(carerId: (state as RegisterUnpaidShiftLoadedState).carerId, start: (state as RegisterUnpaidShiftLoadedState).start, end: event.value.fromColombianToLocalTime())));
  }
